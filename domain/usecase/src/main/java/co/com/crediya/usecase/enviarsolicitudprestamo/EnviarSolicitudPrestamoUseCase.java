...
public class EnviarSolicitudPrestamoUseCase {
    ...
    public EnviarSolicitudPrestamoUseCase(SolicitudRepository solicitudRepository, TipoPrestamoRepository tipoPrestamoRepository, EstadosRepository estadosRepository) {
    ...
    public Mono<Solicitud> guardarSolicitud(Solicitud solicitud) {
        return validarTipoPrestamo(solicitud)
                ...
                .doOnSuccess(success -> logger.info("Solicitud con id " + solicitud.getIdSolicitud() + " guardada correctamente"));
    }
    ...
    private Mono<Solicitud> validarTipoPrestamo(Solicitud solicitud) {
        return tipoPrestamoRepository.existsById(solicitud.getIdTipoPrestamo())
                .flatMap(existe -> {
                    if (!existe) {
                        logger.severe("El tipo de préstamo con id " + solicitud.getIdTipoPrestamo() + " no existe en la base de datos");
    ...
    private Mono<Solicitud> setearEstadoInicial(Solicitud solicitud){
        return estadosRepository.findByNombre(ESTADO_PENDIENTE_DE_REVISION)
                ...
                .doOnError(error -> logger.severe("Estado " + ESTADO_PENDIENTE_DE_REVISION + " no se encuentra en la base de datos"))
                .map(estado -> {
